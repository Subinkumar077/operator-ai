...
server.listen(8080, async () => {
  ...
  try {
    ...
    if (phoneNumber) {
      const incomingNumbers = await twilio.incomingPhoneNumbers.list({ phoneNumber });
      if (incomingNumbers.length > 0) {
        await twilio.incomingPhoneNumbers(incomingNumbers[0].sid).update({
          voiceUrl: tunnel.url,
          voiceMethod: 'POST',
        });
        console.log(`Twilio webhook updated for ${phoneNumber} -> ${tunnel.url}`);
        console.log(`\nReady! Call ${phoneNumber} to test OperatorAI.\n`);
      } else {
        console.warn(`Phone number ${phoneNumber} not found in your Twilio account.`);
        console.log('Please manually set your Twilio Voice webhook to:', tunnel.url);
      }
    } else {
      console.log('No TWILIO_PHONE_NUMBER set. Manually configure your Twilio webhook to:', tunnel.url);
    }

    tunnel.on('close', () => console.log('Tunnel closed'));
  } catch (err) {
    console.error('Tunnel/Twilio setup error:', err.message);
    console.log('You can manually expose port 8080 (e.g. ngrok http 8080) and set the Twilio webhook URL.');
  }
});
